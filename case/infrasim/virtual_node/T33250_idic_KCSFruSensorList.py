...
class T33250_idic_KCSFruSensorList(CBaseCase):
    ...
    def test(self):
        ...
        rsp = qemu_conn.remote_shell('echo {} | sudo -S ipmitool sensor list'.format(self.data['host_password']))
        ...
        if is_match_discrete == None or is_match_degress == None:
            self.result(FAIL, 'Failed on local IPMI command ipmitool sensor list, return: {}'.format(rsp['stdout']))
        else:
