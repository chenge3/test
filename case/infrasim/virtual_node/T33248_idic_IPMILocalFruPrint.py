...
class T33248_idic_IPMILocalFruPrint(CBaseCase):
    ...
    def test(self):
        for obj_rack in self.stack.get_rack_list():
            for obj_node in obj_rack.get_node_list():
                ...
                if is_match == None:
                    self.log('INFO', 'Not failed to issue local IPMI command ipmitool fru')
                else:
                    self.result(FAIL, 'Failed to issue local IPMI command ipmitool fru, return: {}'.format(str_rsp))
