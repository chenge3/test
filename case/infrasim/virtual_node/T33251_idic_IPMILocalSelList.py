...
class T33251_idic_IPMILocalSelList(CBaseCase):
    ...
    def test(self):
        for obj_rack in self.stack.get_rack_list():
            for obj_node in obj_rack.get_node_list():
                ...
                if is_match == None:
                    self.log('INFO', 'Able to issue local IPMI command ipmitool sel list')
                else:
                    self.result(FAIL, 'Failed to issue local IPMI command ipmitool sel list, return: {}'.format(str_rsp))
